...
from cosmos_framework.utils.device import get_gpu_architecture
...
class RunIf:
    ...
    def __new__(
        ...
        requires_hf_model: str | list[str] | None = None,
        ...
        **kwargs: Dict[Any, Any],
        ...
        :param requires_hf_model: Hugging Face model ID or list of model IDs required in the local cache.
        ...
        if requires_hf_model:
            if isinstance(requires_hf_model, str):
                requires_hf_model = [requires_hf_model]
            try:
                from huggingface_hub import try_to_load_from_cache
            except ImportError:
                missing_hf_models = requires_hf_model
            else:
                missing_hf_models = [
                    model
                    for model in requires_hf_model
                    if not isinstance(try_to_load_from_cache(model, "config.json"), str)
                ]
            conditions.append(bool(missing_hf_models))
            reasons.append(f"HF models cached={','.join(missing_hf_models)}")
