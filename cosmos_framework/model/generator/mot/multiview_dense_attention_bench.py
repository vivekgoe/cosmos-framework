...
"""Time ``multiview_dense_attention`` against the paths it is an alternative to.
...
* ``multiview_dense`` -- ``multiview_dense_attention``: three unmasked kernels (same
...
dense one takes the query's own ``(view, frame)`` cell twice, once from each of its two
...
approximation documented on ``multiview_dense_attention``, not a benchmarking artifact.
...
Inference only: ``multiview_dense_attention`` refuses grad, so every row is timed under
...
Example:
    ...
    PYTHONPATH=. python -m cosmos_framework.model.generator.mot.multiview_dense_attention_bench \
...
@dataclass
class BenchConfig:
    """Benchmark ``multiview_dense_attention`` against the flex and dense paths."""
    ...
    A mask geometry, not an attention: the dense row builds no mask and takes its padding
    ...
    def scenario(self, pixel_frames: int, backend: FlexBackend) -> MultiviewScenario:
        ...
        ``multiview_dense_attention`` takes. The scope is pinned to ``"decomposed"``
...
def counted_pairs(scenarios: Sequence[MultiviewScenario], variant: str, per_view_captions: bool = False) -> int:
    ...
    view or -- both being sensor tokens -- a frame, each pair once; ``multiview_dense`` gives
    ...
    for scenario in scenarios:
        ...
        if variant in ("dense_full", "flex_all_views"):
            ...
            neutralised = variant == "multiview_dense" and views == 1
            multiplicity = variant == "multiview_dense" and not neutralised
            gen_pairs = 0
...
def variant_pairs(scenario: MultiviewScenario, variant: str) -> int:
    ...
    * ``flex_decomposed``: the ``F + V - 1`` cells the scope admits, ``S`` tokens each --
      ...
      counted once.
    * ``multiview_dense``: ``F + V`` cells, because its two passes each take that shared
    ...
    cells = {
        ...
        "multiview_dense": scenario.latent_frames_per_view + scenario.num_views,
    }[variant]
...
def run_scenario(
    ...
    calls["multiview_dense"] = _timed_or_trained(config, lambda: multiview_dense_attention(*packs, plan=plan))
